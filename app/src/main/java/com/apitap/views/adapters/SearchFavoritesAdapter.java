...
public class SearchFavoritesAdapter extends ArrayAdapter<String> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        textName.setText(array.get(position).getName());
        textCompany.setText("By "+array.get(position).getStoreName());
        textCost.setText("$ "+array.get(position).getActualPrice());
        //Picasso.with(context).load(ATPreferences.readString(context, Constants.KEY_IMAGE_URL) +array.get(position).getImagesData().get(0).getImageUrl()).into(image);
