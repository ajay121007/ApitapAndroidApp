...
public class FavoriteStoreAdapter extends BaseAdapter {
    ...
    List<ShopAsstSearchBean.RESULT_> arrayList = new ArrayList<>();



    public FavoriteStoreAdapter(Activity activity, List<ShopAsstSearchBean.RESULT_> arrayList, TabLayout tabLayout, ArrayList<String> favoritemerchants) {
    ...
    private void setData(final int position, ViewHolders holder) {
        holder.merchant_name.setText(arrayList.get(position).getPC().get(0).get11470());
        ...
        Picasso.with(activity).load(ATPreferences.readString(activity, Constants.KEY_IMAGE_URL) + arrayList.get(position)
