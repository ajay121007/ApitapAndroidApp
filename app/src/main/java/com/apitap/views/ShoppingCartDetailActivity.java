    ...
    private void getBundles() {
        ...
        if (b != null) {
            ...
            try {
                ...
                txt_date.setText("Date: " + DateTime);
                e.printStackTrace();
            }
            txt_time.setText("Time: " + Utils.getTimeFromInvoice(DateTime));
            Picasso.with(mActivity).load(ATPreferences.readString(mActivity, Constants.KEY_IMAGE_URL) + companyLogo).into(logo_merchant);
            Picasso.with(mActivity).load(ATPreferences.readString(mActivity, Constants.KEY_IMAGE_URL) + companyLogo).into(storeImage);
    ...
    public void afterSubmit() {
        ...
        ModelManager.getInstance().getShoppingManager().getShoppingCart(ShoppingCartDetailActivity.this, Operations.makeJsonPayShoppingCart
                (ShoppingCartDetailActivity.this, merchantId, shoppingCompBean.getShoppingCartId(), total_Amount_Is, tip_is, shipping_id,
                        deliver_Id, card_token, total_Amount_Is, tax, shoppingCompBean, Choice_Id, Choice_Price, arrchoice, specialInstr));
